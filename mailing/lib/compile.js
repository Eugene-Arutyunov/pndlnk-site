...
// Class-based renderer даёт доступ к this.parser.parseInline()
// что необходимо для корректной обработки инлайн-токенов в marked v14
class EmailRenderer extends Renderer {
  heading(token) {
    ...
    const style = styles[token.depth] || 'font-size:16px;font-weight:700;margin:16px 0 8px;color:#232526;';
  ...
  link(token) {
    const text = this.parser.parseInline(token.tokens);
    return `<a href="${token.href}" style="color:#232526;text-decoration:underline;">${text}</a>`;
  }

  image(token) {
    return `<img src="${token.href}" alt="${token.text || ''}" style="max-width:100%;height:auto;display:block;margin:0 0 16px;border-radius:4px;" />\n`;
  }

  list(token) {
    const tag = token.ordered ? 'ol' : 'ul';
    const items = token.items.map((item) => this.listitem(item)).join('');
    return `<${tag} style="padding-left:24px;margin:0 0 16px;">${items}</${tag}>\n`;
  }
  ...
  code(token) {
    return `<pre style="background:#E6E4E4;padding:16px;border-radius:4px;overflow:auto;font-family:monospace;font-size:14px;margin:0 0 16px;"><code>${token.text}</code></pre>\n`;
  }

  codespan(token) {
    return `<code style="background:#E6E4E4;padding:2px 6px;border-radius:3px;font-family:monospace;font-size:14px;">${token.text}</code>`;
  }
}
...
function compile(contentFile) {
  ...
  const htmlContent = marked(markdown, { renderer: new EmailRenderer() });
