...
| `case-kids-summary.css` | Саммари кейса: контекст/запрос, результат, процесс. Кандидат в стандартный саммари всех кейсов | Додо Kids |
...
- Внутри dkcp-page.css пока живут «чужие» виджеты: каталог проектов (`.project-card`, используется на /projects и главной) и промо-строки продуктов (`.promo-row`, используется на /products) — кандидаты на вынос в widgets/
...
1. **Близнецы палитры**: `--ids__color-gray-800` (45, 47, 48 — тёмный surface) и `--ids__color-gray-850` (44, 44, 44 — основной текст) визуально неразличимы — слить? Решение за дизайнером.
2. **Тёмные блоки не переопределяют** `--ids__ink`, `--ids__code`, `--ids__inverted-text` — исторически. Сейчас внутри тёмных блоков эти роли не используются, поэтому ничего не ломается; при первом использовании нужно добавить осветлённые варианты (потребуются новые ступени палитры).
3. **Брейкпоинт-хвосты**: конвенция — единственный брейкпоинт `width < 768px`; в ksc-open.css и кейсах остались одиночные 700/720/900/1100px и `max-width`-записи — трогать только с визуальной проверкой (off-by-one на границе).
4. **Виджеты в dkcp-page.css**: каталог проектов и промо-строки продуктов ждут выноса в `widgets/` при следующем заходе на /projects и /products.
